  ...
  <div class="min-h-screen bg-gray-50 flex">
    ...
    >
      ...
      <div class="h-16 flex items-center justify-between px-4 border-b border-gray-200">
        <h1 v-if="!sidebarCollapsed" class="text-xl font-bold text-primary-600">Flowhive</h1>
      ...
      <nav class="flex-1 overflow-y-auto py-4">
        ...
        <div class="px-3 mb-4">
          <RouterLink
            ...
            :class="isActive('/') && !isActive('/workspaces') ? 'bg-primary-50 text-primary-700' : 'text-gray-700 hover:bg-gray-100'"
          >
          ...
          <RouterLink
            ...
            :class="isActive('/my-tasks') ? 'bg-primary-50 text-primary-700' : 'text-gray-700 hover:bg-gray-100'"
          >
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <RouterLink
            ...
            to="/analytics"
            class="flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors mt-1"
            :class="isActive('/analytics') ? 'bg-primary-50 text-primary-700' : 'text-gray-700 hover:bg-gray-100'"
          >
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="!sidebarCollapsed" class="px-3">
          <div class="flex items-center justify-between px-3 mb-2">
            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Workspaces</h3>
            <button
          ...
          <div v-if="workspaceStore.loading" class="px-3 py-2 text-xs text-gray-500">Loading...</div>
          <div v-else-if="workspaceStore.workspaces.length === 0" class="px-3 py-2 text-xs text-gray-500">
            No workspaces yet
          </div>
          <div v-else class="space-y-1">
            ...
            >
              ...
              <div v-if="expandedWorkspaces.has(workspace.id)" class="ml-6 mt-1 space-y-0.5">
                <div v-if="loadingProjects[workspace.id]" class="px-3 py-1 text-xs text-gray-500">
                  Loading projects...
                </div>
                <div v-else-if="workspaceProjects[workspace.id]?.length === 0" class="px-3 py-1 text-xs text-gray-500">
                  No projects
                </div>
                <RouterLink
                  v-else
                  ...
                  :class="isProjectActive(project.id) ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'"
                >
                  <div
      ...
      <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6">
        ...
        <div class="flex items-center space-x-4">
          ...
          >
            ...
            <span v-if="unreadCount > 0" class="absolute top-1 right-1 block h-2 w-2 rounded-full bg-red-500"></span>
          </button>
        </div>
      </header>
    ...
    <!-- Create Workspace Modal -->
              >
                Logout
    <!-- Create Workspace Modal -->
    <div v-if="showCreateWorkspace" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click="showCreateWorkspace = false">
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" @click.stop>
        <div class="mt-3">
          ...
          <form @submit.prevent="handleCreateWorkspace" class="space-y-4">
            ...
            <div>
              <label for="workspaceDescription" class="block text-sm font-medium text-gray-700">Description</label>
              <textarea
...
onMounted(async () => {
  ...
  if (projectId && route.name === 'project') {
    ...
    for (const workspace of workspaceStore.workspaces) {
      ...
      const hasProject = workspaceProjects.value[workspace.id]?.some(p => p.id === parseInt(projectId as string))
      if (hasProject) {
...
watch(() => route.params.id, async (newId) => {
  if (newId && route.name === 'project') {
    for (const workspace of workspaceStore.workspaces) {
      ...
      const hasProject = workspaceProjects.value[workspace.id]?.some(p => p.id === parseInt(newId as string))
      if (hasProject) {
        expandedWorkspaces.value.add(workspace.id)
        break
      }
...
const isProjectActive = (projectId: number) => {
  return route.name === 'project' && parseInt(route.params.id as string) === projectId
}
